...
| [shared/.../ui/chart/ChartEditorScreen.kt](../../../shared/src/commonMain/kotlin/io/github/b150005/skeinly/ui/chart/ChartEditorScreen.kt) | Compose Multiplatform screen (~1800 LOC). Private composables: `EditorBody`, `EditorCanvas` (dispatcher) → `RectEditorCanvas` / `PolarEditorCanvas`, `RectEditorAccessibilityOverlay` (ADR-025 R1b — per-row overlay + in-row adjustable cell cursor, same M5 content space; see Invariant 9), `LayerDrawerContent`, `LayerRow`, `ParameterInputDialog`, `PolarExtentsDialog`, `ResizeChartDialog`, `ChartMetadataMenu`. `RectEditorCanvas` wraps the `editorCanvas` Canvas in a 2-axis scroll container sized at `gridW×gridH×effectiveCell` (M5 / WCAG 2.5.8 — see Invariant 8); helpers `fittedCell`, `centeredLayout`, const `MAX_EDITOR_ZOOM_SCALE` |
...
| [shared/.../domain/chart/WcagTargetSize.kt](../../../shared/src/commonMain/kotlin/io/github/b150005/skeinly/domain/chart/WcagTargetSize.kt) | M5 (rect only). `minimumScale(...)` → the minimum zoom factor so a fitted cell renders ≥ 24dp/pt (WCAG 2.5.8). Pure, single source of truth for both platforms; `1.0` for small/default grids. **UI consumer wired** (M5 zoom slice): both `RectEditorCanvas` (Compose) and `scrollableRectCanvas` (SwiftUI) call it to derive `effectiveCell` — see Invariant 8 |
...
| Path | Role |
|---|---|
| [iosApp/iosApp/Screens/ChartEditorScreen.swift](../../../iosApp/iosApp/Screens/ChartEditorScreen.swift) | SwiftUI mirror. `EditorCanvasView` (→ `scrollableRectCanvas` / `fittingPolarCanvas`; rect canvas in a 2-axis `ScrollView` sized at `gridW×gridH×effectiveCell` per Invariant 8; rect canvas `.accessibilityHidden(true)` + `.overlay { ChartEditorAccessibilityOverlay }` per Invariant 9), `SymbolPaletteView`, `ReflectionAxisPickBanner`, layer sheet, resize sheet, polar picker, parameter sheet, discard confirm |
| [iosApp/iosApp/Screens/ChartEditorAccessibility.swift](../../../iosApp/iosApp/Screens/ChartEditorAccessibility.swift) | ADR-025 R1b — invisible per-row a11y overlay (`ChartEditorAccessibilityOverlay`) + private `EditorRowAccessibilityCell`. Same forward `cellGeometry` math as the visual Canvas (no inverse transform). Adjustable cell cursor + place/erase custom action. See Invariant 9 |
...
6. **Compose vs SwiftUI divergences**:
   ...
   - Compose has separate `EditorCanvas` (dispatcher) → `RectEditorCanvas` / `PolarEditorCanvas` selected via `when (state.draftExtents)`. iOS `EditorCanvasView` is one struct that branches in `body` to `scrollableRectCanvas` / `fittingPolarCanvas`.
...
8. **WCAG 2.5.8 zoom — one coordinate space, no render-only transform** (M5 zoom slice): the rect editor Canvas is laid out at `contentSize = (gridW × effectiveCell, gridH × effectiveCell)` where `effectiveCell = fittedCell × WcagTargetSize.minimumScale(...)`, inside a 2-axis **layout** scroll container (Compose `horizontalScroll`+`verticalScroll`; SwiftUI `ScrollView([.horizontal,.vertical])` + Canvas `.frame(width:height:)`). Canvas drawn size = `max(content, viewport)`; the grid is centred via `centeredLayout` origin so **content ≤ viewport ⇒ no scroll ⇒ pixel-identical to the pre-M5 fitting canvas** (default 8×8 → `minimumScale` returns `1.0`, zero behaviour change, `P1_chart_editor.yaml` unaffected). Rendering AND `GridHitTest.hitTest` run in the single Canvas content space — scroll modifiers are layout modifiers, so the tap offset is already content-space; **NEVER reintroduce `graphicsLayer` / `.scaleEffect` / `rememberTransformableState` / a `toCanvasSpace` inverse transform** (the rejected pattern hit-tests the wrong cell at scale≠1 and only visually meets 2.5.8 — see CLAUDE.md `### Accessibility deferrals`). Rect only — the larger-Canvas path is gated on `extents is Rect`; the polar canvas keeps the pre-M5 viewport-fit path verbatim (polar zoom stays Phase 35.2+). `MAX_EDITOR_ZOOM_SCALE = 40.0` (Compose) / `maxEditorZoomScale = 40.0` (SwiftUI) caps the auto-zoom — it is NOT a user pinch ceiling (there is no pinch; the ≥24dp guarantee is structural via min cell + scroll, so no new i18n). The scale `WcagTargetSize.minimumScale` needs is **density-invariant** in the non-floored regime (`24 × gridDimension / viewportDp` on the tighter axis — density cancels between the 24dp target and the fitted cell); for the editor's hard `MAX_GRID_DIMENSION` (256) the worst realistic editor viewport (tighter axis ≳ 160dp, incl. a 1/3 split) needs ≤ ~38.4, so the 40 cap keeps the ≥24dp floor structurally intact for every buildable grid on a real device. Only a sub-realistic < ~153dp tighter-axis viewport with a 256-cell grid clips, degrading gracefully to a Canvas-size bound (cells < 24dp but still scroll-pannable, never an unbounded Canvas).

9. **Editor accessibility — additive overlay, same M5 content space, no inverse transform** (ADR-025 R1b, audit B3 closed): the rect editor exposes its grid to TalkBack / VoiceOver via an **invisible per-row overlay** layered on the SAME content-sized Box (Compose) / `.overlay` of the Canvas-sized `ScrollView` content (SwiftUI) that the visual Canvas occupies — sharing M5's `centeredLayout` + `effectiveCell` math, **no inverse transform, no second coordinate space** (Invariant 8 spirit preserved by construction). Each grid row is one focusable accessibility element whose spoken text is built by the pure shared `ChartAccessibility.spokenLabel(rowDescriptor, A11yStrings, symbolName)` (position + run-length symbol summary, no progress section — the editor has no project context). The row element is **adjustable**: Compose attaches `progressBarRangeInfo` + `setProgress` (`current=cursorX`, `range=minX..maxX`, `steps=colCount-2`) and SwiftUI attaches `.accessibilityAdjustableAction` — both wire to a per-row cursor state map (`mutableStateMapOf<Int,Int>` keyed by chart-y in Compose, `@State [Int32: Int32]` with `.id(rect)` reset on SwiftUI). The cursor announcement is the shared `ChartAccessibility.spokenCellLabel(cellDescriptor, CellA11yStrings, symbolName)` — read into Compose `stateDescription` / SwiftUI `accessibilityValue` so VoiceOver re-announces it on every cursor move. A single named **custom action** routes to the existing `ChartEditorEvent.PlaceCell(cursorX, cursorY)` — the label flips between "Place &lt;localized symbol name&gt;" (palette selected) and "Erase" (no selection) via `ChartAccessibility.placeOrEraseActionLabel(...)`, faithfully mirroring the touch affordance because the VM already routes `selectedSymbolId == null` to an immediate erase. The visual Canvas keeps its `testTag("editorCanvas")` Maestro landmark (the testTag-only Compose Canvas has no role / contentDescription / click semantic so it is not TalkBack-focusable by construction — same R1a precedent); iOS sets `.accessibilityHidden(true)` on the inner Canvas. The overlay rows carry **no pointer-input modifier** so touch + scroll pass through to the Canvas underneath for sighted users. Rect only — the polar editor a11y is gated to Phase 35.2+ in lockstep with the polar editor UX itself (ADR-025 §e + M5 polar-zoom deferral). **NEVER reintroduce per-cell semantic nodes** (up to 65 k stops is unusable; ADR-025 §a rejected) — the row + adjustable cursor is the chosen, craft-aligned, ≤256-stop primitive.
...
| Item | Pointer |
|---|---|
| **Zoom (M5, WCAG 2.5.8)** — ✅ **shipped** (rect). Foundation `WcagTargetSize.kt` + 13 commonTest; UI = scroll-container + `effectiveCell` on both platforms. Behaviour + the rejected-pattern guard are now Invariant 8 above (load-bearing). | Invariant 8 · CLAUDE.md → "Accessibility deferrals" |
| **Polar zoom** — the polar canvas still viewport-fits with no WCAG min-cell zoom (large round charts can drop below 24dp/pt). Deferred with the rest of polar authoring. | Phase 35.2+ |
