...
| [shared/.../ui/chart/ChartEditorScreen.kt](../../../shared/src/commonMain/kotlin/io/github/b150005/skeinly/ui/chart/ChartEditorScreen.kt) | Compose Multiplatform screen (~1650 LOC). Private composables: `EditorBody`, `EditorCanvas` (dispatcher) → `RectEditorCanvas` / `PolarEditorCanvas`, `LayerDrawerContent`, `LayerRow`, `ParameterInputDialog`, `PolarExtentsDialog`, `ResizeChartDialog`, `ChartMetadataMenu`. `RectEditorCanvas` wraps the `editorCanvas` Canvas in a 2-axis scroll container sized at `gridW×gridH×effectiveCell` (M5 / WCAG 2.5.8 — see Invariant 8); helpers `fittedCell`, `centeredLayout`, const `MAX_EDITOR_ZOOM_SCALE` |
...
| Path | Role |
|---|---|
| [iosApp/iosApp/Screens/ChartEditorScreen.swift](../../../iosApp/iosApp/Screens/ChartEditorScreen.swift) | SwiftUI mirror. `EditorCanvasView` (→ `scrollableRectCanvas` / `fittingPolarCanvas`; rect canvas in a 2-axis `ScrollView` sized at `gridW×gridH×effectiveCell` per Invariant 8), `SymbolPaletteView`, `ReflectionAxisPickBanner`, layer sheet, resize sheet, polar picker, parameter sheet, discard confirm |
