...
| [shared/.../domain/chart/WcagTargetSize.kt](../../../shared/src/commonMain/kotlin/io/github/b150005/skeinly/domain/chart/WcagTargetSize.kt) | M5 foundation (rect only). `minimumScale(...)` → the minimum zoom factor so a fitted cell renders ≥ 24dp/pt (WCAG 2.5.8). Pure, single source of truth for both platforms; `1.0` for small/default grids. UI consumer (scroll + effective cell) not yet wired — see Deferred § Zoom |
...
| Item | Pointer |
|---|---|
| **Zoom (M5, WCAG 2.5.8)** — *foundation landed* (`WcagTargetSize.kt` + 13 commonTest, rect only). *UI slice pending*: must use a **layout-driven scroll container + effective cell size** (`effectiveCell = fittedCell × WcagTargetSize.minimumScale(...)`; Canvas laid out at `gridW×gridH×effectiveCell` inside a 2-axis scroll container; rendering + hit-test in one content coordinate space, **no inverse transform**). **Do NOT** reuse the chart-viewer's `rememberTransformableState`/`graphicsLayer` (Compose) or `.scaleEffect`+`toCanvasSpace` (SwiftUI) render-only-transform pattern: a 3-reviewer pass found it (a) hit-tests the wrong cell when scale≠1 — Compose `graphicsLayer` is render-only so `pointerInput` gets layout-space coords with no inversion; SwiftUI `.scaleEffect` is center-anchored and the viewer's `toCanvasSpace` omits the `cx·(1−1/scale)` term — and (b) on Compose does not enlarge the real touch target so WCAG 2.5.8 is only *visually* met. The viewer carries the same latent defect (masked: scale starts 1, read-only). Default/small grids (content ≤ viewport) must stay pixel-identical (no scroll) so `e2e/flows/android/P1_chart_editor.yaml` is unaffected. Rect only — polar zoom stays Phase 35.2+. | CLAUDE.md → Tech Debt Backlog → "Accessibility deferrals" |
