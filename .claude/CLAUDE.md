...
- **i18n sweep across remaining shared screens**: Phase 33.1 landed the `compose.components.resources` plumbing (+ chart editor reference), Phase 33.1.1 migrated `SymbolGalleryScreen` (+ fixed the AGP 9.x CMP-resource-packaging bug that Phase 33.1 shipped with latent — see CI Known Limitations), Phase 33.1.2 migrated `ChartViewerScreen`, and Phase 33.1.3 migrated `ProjectListScreen` (+ introduced the `hint_` prefix and the "any shared-screen migration that changes a user-visible literal hit by a Maestro regex selector needs a testTag preflight" rule — see Phase 33.1.3 Completed entry for the commit `b0fb3d8` search-field follow-up fix). The following shared Compose screens still hardcode English and need the same `stringResource(Res.string.*)` treatment: `PatternLibraryScreen`, `PatternEditScreen`, `ProjectDetailScreen`, `OnboardingScreen`, `SettingsScreen`, `DiscoveryScreen`, `SharedContentScreen`, `SharedWithMeScreen`, `ActivityFeedScreen`, `LoginScreen`, `ProfileScreen`, `CommentSection` (13 remaining). Each migration adds its own domain keys to all 5 i18n sources and should be cut as its own PR. **Note for Maestro flows**: once a screen's title becomes i18n'd, any existing `assertVisible: "<English title>"` must be replaced with the screen's stable testTag (as done for `P1_symbol_gallery.yaml`) — otherwise the flow fails the moment CMP resolves to the Japanese locale or the (now-asynchronous) resource takes a beat to load. Also: Maestro `tapOn: "text"` uses regex; literal `...` matches 3 chars but Unicode `\u2026` (`…`) is 1 char — any U+2026 ellipsis introduction (common for placeholders per the `state_loading` style) must either update the flow's regex or route through a testTag.
