...
| `upload-keystore.jks` (overridable via `KEYSTORE_FILE` in `local.properties`) | Optional (Release builds only) | none — generate locally or share via Bitwarden | Android Release AAB upload key (Play App Signing uses this to verify your AAB uploads; Google manages the actual app signing key) | Generate via `keytool -genkey -v -keystore upload-keystore.jks ...` per [release-secrets.md §7](./docs/en/ops/release-secrets.md), OR Android Studio → Build → Generate Signed Bundle / APK → new keystore. Default path is `upload-keystore.jks` at repo root; override via `KEYSTORE_FILE=<path>` in `local.properties` for a custom name |
...
**Keystore default + custom filename**: the Android upload keystore path is configurable, but the project has a **canonical default**: `upload-keystore.jks` at the repo root.

- **Why `upload-keystore.jks`**: Skeinly publishes via Google Play App Signing (the Play Console manages the actual signing key server-side; the developer's local key is the *upload key* that authenticates each AAB upload). The `upload-keystore.jks` filename matches the convention used in [`docs/en/ops/release-secrets.md §7`](./docs/en/ops/release-secrets.md) (keytool generation + SHA-1 verification commands) and aligns with the Google Android Studio docs example (`your-upload-keystore.jks`). Following the runbook with no overrides drops a file at exactly this path, which the build picks up zero-config.
- **Resolution order**: (1) `KEYSTORE_FILE=<path>` in `local.properties` → (2) `KEYSTORE_FILE` env var (used by CI per `release.yml`) → (3) default `upload-keystore.jks` at repo root. Relative paths resolve against the repo root; absolute paths are used verbatim. CI's `KEYSTORE_FILE` is set after decoding the `KEYSTORE_BASE64` secret to `$RUNNER_TEMP/upload-keystore.jks`, so the resolution chain is uniform local ↔ CI.
- **Override only when needed**: legacy keystores named `skeinly-release.jks`, `keystore.jks`, `release.keystore`, etc. continue to work by setting `KEYSTORE_FILE=<your-filename>` in `local.properties` — no rename required.
...
| `upload-keystore.jks`（`local.properties` の `KEYSTORE_FILE` で上書き可） | 任意（Release ビルド時のみ） | テンプレートなし — ローカル生成 or Bitwarden 経由共有 | Android Release AAB のアップロード鍵（Play App Signing が AAB アップロードの authenticity 検証に使用; Google 側が実際のアプリ署名鍵をサーバーサイドで管理） | [release-secrets.md §7](./docs/en/ops/release-secrets.md) 記載の `keytool -genkey -v -keystore upload-keystore.jks ...` で生成、もしくは Android Studio → Build → Generate Signed Bundle / APK → 新規 keystore。デフォルトは repo ルート `upload-keystore.jks`、`local.properties` の `KEYSTORE_FILE=<path>` で任意名に上書き可 |
...
**Keystore のデフォルトとカスタマイズ**: Android のアップロード keystore のパスは設定可能ですが、プロジェクト規定の **canonical default は `upload-keystore.jks`** (リポジトリルート)。

- **`upload-keystore.jks` を選んだ理由**: Skeinly は Google Play App Signing 経由で配信しているため (Play Console 側が実際のアプリ署名鍵をサーバーサイドで管理)、開発者がローカルに持つのは AAB アップロードを認証する *upload key* です。[`docs/en/ops/release-secrets.md §7`](./docs/en/ops/release-secrets.md) の `keytool` 生成コマンド + SHA-1 検証コマンド + Google 公式 Android Studio ドキュメントの placeholder (`your-upload-keystore.jks`) と一致する命名です。runbook 通りに生成すれば何もしなくてもリポジトリルートに `upload-keystore.jks` が落ちて、Gradle が zero-config で拾います。
- **解決順序**: (1) `local.properties` の `KEYSTORE_FILE=<path>` → (2) `KEYSTORE_FILE` 環境変数 (CI の `release.yml` が使用) → (3) デフォルト `upload-keystore.jks` (repo ルート)。相対パスはリポジトリルート基準、絶対パスはそのまま使用。CI は `KEYSTORE_BASE64` secret を `$RUNNER_TEMP/upload-keystore.jks` にデコードしてから `KEYSTORE_FILE` をセットするため、ローカル ↔ CI で解決ロジックが完全に統一されています。
- **上書きは必要な場合のみ**: 過去の慣習で `skeinly-release.jks` / `keystore.jks` / `release.keystore` 等のファイル名を使っている場合は `local.properties` に `KEYSTORE_FILE=<ファイル名>` を追加すれば動きます — リネーム不要。
