...
Deno.test("TARGET_TYPES matches DB CHECK constraint (migration 031)", () => {
    assertEquals(TARGET_TYPES, ["pattern", "comment", "suggestion", "suggestion_comment"]);
});
...
Deno.test("redactReason truncates text past the cap and annotates length", () => {
    ...
    assertEquals(out.slice(0, ISSUE_BODY_REASON_PREVIEW_CHARS), "x".repeat(ISSUE_BODY_REASON_PREVIEW_CHARS));
});
