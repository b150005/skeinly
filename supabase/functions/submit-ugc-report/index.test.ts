...
import {
  fakeBearerForUser,
  generateTestGithubAppCredentials,
  installFakeFetch,
  jsonResponse,
} from "./_fakes.ts";
...
function installStandardFakes(opts: FakeOptions = {}) {
  ...
  const issueHtmlUrl = opts.issueHtmlUrl ??
    "https://github.com/b150005/skeinly/issues/7";
  const issueStatus = opts.issueStatus ?? 201;
  const tokenExpiresAt = opts.tokenExpiresAt ??
  ...
  return installFakeFetch((req) => {
    ...
    if (url.includes("/repos/b150005/skeinly/issues")) {
      ...
      return jsonResponse(
        { number: issueNumber, html_url: issueHtmlUrl },
        issueStatus,
      );
    }
...
Deno.test("extractUserIdFromAuthHeader returns null when payload has no sub", () => {
  const payload = btoa(JSON.stringify({ role: "authenticated" })).replace(
    /=/g,
    "",
  );
  assertEquals(
    extractUserIdFromAuthHeader(`Bearer header.${payload}.sig`),
    null,
  );
});
...
Deno.test("handleRequest GET returns 405", async () => {
  ...
  await setEnvFromTestCreds();
  const resp = await handleRequest(
    new Request("https://t/", { method: "GET" }),
  );
...
Deno.test("handleRequest returns UNAUTHORIZED when Authorization header missing", async () => {
  ...
  await setEnvFromTestCreds();
  const resp = await handleRequest(
    buildRequest(validBody(), TEST_USER_A, { authHeader: null }),
  );
  const json = await resp.json();
...
Deno.test("handleRequest rejects unknown target_type with VALIDATION_FAILED", async () => {
  ...
  await setEnvFromTestCreds();
  const resp = await handleRequest(
    buildRequest(validBody({ target_type: "screenshot" })),
  );
  const json = await resp.json();
...
Deno.test("handleRequest rejects non-UUID target_id", async () => {
  ...
  await setEnvFromTestCreds();
  const resp = await handleRequest(
    buildRequest(validBody({ target_id: "not-a-uuid" })),
  );
  const json = await resp.json();
  assertEquals(json.code, "VALIDATION_FAILED");
});
...
Deno.test("handleRequest rate limit is per-user (USER_A and USER_B independent)", async () => {
  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      await handleRequest(
        buildRequest(validBody({ reason: `a${i}` }), TEST_USER_A),
      );
    }
    // USER_B is independent
    const resp = await handleRequest(
      buildRequest(validBody({ reason: "b1" }), TEST_USER_B),
    );
    const json = await resp.json();
...
Deno.test("handleRequest happy path returns ok with report_id + github_issue_url", async () => {
  ...
  try {
    ...
    assertEquals(
      json.github_issue_url,
      "https://github.com/b150005/skeinly/issues/42",
    );
  } finally {
    fake.restore();
  }
...
Deno.test("handleRequest applies label 'ugc-report' to GitHub Issue", async () => {
  ...
  try {
    ...
    const issueCall = fake.calls.find((c) =>
      c.url.includes("/repos/b150005/skeinly/issues")
    );
...
Deno.test("handleRequest UPDATEs github_issue_url after Issue POST success", async () => {
  ...
  try {
    ...
    assert(
      ...
      "expected PATCH to ugc_reports for github_issue_url backfill",
    );
    const payload = JSON.parse(updateCall!.body ?? "{}");
    assertEquals(
      payload.github_issue_url,
      "https://github.com/b150005/skeinly/issues/99",
    );
  } finally {
    fake.restore();
  }
...
Deno.test("handleRequest treats GitHub Issue failure as best-effort: returns ok + github_issue_url=null", async () => {
  ...
  try {
    ...
    assertEquals(
      json.ok,
      true,
      "DB row is canonical — request should still succeed",
    );
