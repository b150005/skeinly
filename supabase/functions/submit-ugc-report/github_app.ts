// Pre-alpha A1/A5 (ADR-021 §D3) — GitHub App authentication + Issue creation.
//
// DUPLICATED from supabase/functions/submit-bug-report/github_app.ts to keep
// each Edge Function self-contained at deploy time. Supabase Edge Functions
// upload the function directory as a bundle; relative imports across
// function dirs (e.g. `../submit-bug-report/github_app.ts`) couple deploy
// commands and break the "deploy submit-ugc-report independently" property.
// The shared-`_shared/` refactor is a Tech Debt item slated for pre-Phase-40
// GA when more functions accumulate around the same GitHub App pattern.
// Until then, this file MUST stay byte-for-byte identical to its sibling.
//
