...
class SignInUseCaseTest {
    ...
    @Test
    fun `sign in with valid credentials returns Success`() = runTest {
        val result = signIn("user@example.com", "password123")
        assertIs<UseCaseResult.Success<Unit>>(result)
    }

    @Test
    fun `sign in updates auth state to Authenticated`() = runTest {
        signIn("user@example.com", "password123")
        assertEquals("test-user-id", fakeAuth.getCurrentUserId())
    }

    @Test
    fun `sign in failure returns Failure with Unknown error`() = runTest {
