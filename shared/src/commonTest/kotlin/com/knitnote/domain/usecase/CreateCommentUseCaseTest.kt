...
class CreateCommentUseCaseTest {
    ...
    @Test
    fun `returns failure when body is empty`() = runTest {
        val authRepo = FakeAuthRepository()
        authRepo.setAuthState(AuthState.Authenticated("user-1", "test@test.com"))
    ...
    @Test
    fun `creates comment successfully`() = runTest {
        val authRepo = FakeAuthRepository()
        authRepo.setAuthState(AuthState.Authenticated("user-1", "test@test.com"))
    ...
    @Test
    fun `returns failure when body exceeds max length`() = runTest {
        val authRepo = FakeAuthRepository()
        authRepo.setAuthState(AuthState.Authenticated("user-1", "test@test.com"))
        val useCase = CreateCommentUseCase(FakeCommentRepository(), authRepo)
    ...
    @Test
    fun `returns failure when target ID is not a UUID`() = runTest {
        val authRepo = FakeAuthRepository()
        authRepo.setAuthState(AuthState.Authenticated("user-1", "test@test.com"))
        val useCase = CreateCommentUseCase(FakeCommentRepository(), authRepo)
    ...
    @Test
    fun `stores comment in repository`() = runTest {
        val authRepo = FakeAuthRepository()
        authRepo.setAuthState(AuthState.Authenticated("user-1", "test@test.com"))
        val commentRepo = FakeCommentRepository()
        val useCase = CreateCommentUseCase(commentRepo, authRepo)
