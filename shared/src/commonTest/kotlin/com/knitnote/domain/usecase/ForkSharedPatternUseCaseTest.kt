...
class ForkSharedPatternUseCaseTest {
    ...
    @Test
    fun `returns failure when permission is VIEW`() = runTest {
        val authRepo = FakeAuthRepository()
        authRepo.setAuthState(AuthState.Authenticated("user-1", "test@test.com"))
    ...
    @Test
    fun `returns failure when share belongs to another user`() = runTest {
        val authRepo = FakeAuthRepository()
    ...
    @Test
    fun `returns failure when share is declined`() = runTest {
        val authRepo = FakeAuthRepository()
        authRepo.setAuthState(AuthState.Authenticated("user-1", "test@test.com"))
        val shareRepo = FakeShareRepository()
    ...
    @Test
    fun `returns failure when source pattern not found`() = runTest {
        val authRepo = FakeAuthRepository()
        authRepo.setAuthState(AuthState.Authenticated("user-1", "test@test.com"))
        val shareRepo = FakeShareRepository()
    ...
    @Test
    fun `successfully forks pattern and creates project`() = runTest {
        val authRepo = FakeAuthRepository()
        authRepo.setAuthState(AuthState.Authenticated("user-1", "test@test.com"))
        val shareRepo = FakeShareRepository()
        shareRepo.addShare(forkableShare)
    ...
    @Test
    fun `forked project links to forked pattern not original`() = runTest {
        val authRepo = FakeAuthRepository()
        authRepo.setAuthState(AuthState.Authenticated("user-1", "test@test.com"))
        val shareRepo = FakeShareRepository()
        shareRepo.addShare(forkableShare)
