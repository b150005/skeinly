...
private func paletteSymbolAccessibilityLabel(for def: SymbolDefinition) -> String {
    ...
    // `SymbolDefinition.localizedLabel(locale:)` (Bridging/SymbolDefinition+Localized.swift)
    // mirroring the Y3 Kotlin half. Predicate matches `startsWith("ja",
    // ignoreCase = true)` exactly.
    let locale = Locale.current.language.languageCode?.identifier ?? "en"
    let symbolName = def.localizedLabel(locale: locale)
    return String(
