...
| Support URL | `https://github.com/b150005/knit-note/issues` |
...
The same APNs key authorizes pushes from Knit Note's Supabase Edge Function for **both production and TestFlight** builds — no separate keys needed per environment.
...
5. Bundle ID: select `io.github.b150005.knitnote` (must already exist from Phase A0a-1).
6. SKU: `knitnote-001`.
...
3. Localizations:
   ...
   - Japanese: Display Name `Knit Note Pro` (brand name kept identical across locales).
...
**Knit Note's hosting situation**: GitHub Pages serves this project at `https://b150005.github.io/knit-note/` (a project page under user `b150005`). The AASA must be at the apex of the associated domain — `https://b150005.github.io/.well-known/apple-app-site-association`. That apex is the **GitHub user site** for `b150005`, which lives in a separate repo `b150005/b150005.github.io`. The project repo `b150005/knit-note` cannot serve AASA at that apex.
...
- Add `applinks:b150005.github.io` to the app entitlements; restrict path matches to `/knit-note/share/*` etc. so the user site doesn't accidentally route other paths into the app.

**Option B — Custom domain (paid, recommended for v1.0)**
- Buy `knitnote.app` or similar (Cloudflare Registrar at-cost, ~$10/yr).
- Configure GitHub Pages CNAME → custom domain.
- AASA at `https://knitnote.app/.well-known/apple-app-site-association`.
- Cleaner branding for production launch.

**Option C — Defer Universal Links to v1.0 (no domain investment for alpha)**
- Stay on `knitnote://share/<token>` URI scheme only for alpha.
...
Replace `TEAMID` with your 10-char Apple Developer Team ID. Adjust the `/knit-note/...` prefix if you choose Option B (drop the prefix on a custom domain).
...
- [ ] Apple Developer App ID `io.github.b150005.knitnote` exists with 4 capabilities enabled (Sign In with Apple, Push Notifications, Associated Domains, In-App Purchase)
...
- [ ] App Store Connect app `Knit Note` created with bundle ID `io.github.b150005.knitnote`
- [ ] Subscription Group `Knit Note Pro` created
- [ ] 2 IAP products created: `knitnote.pro.monthly` ($3.99/¥600) and `knitnote.pro.yearly` ($24.99/¥3,800), both with 7-day free trial
