...
**WHAT**: Shared secret used to sign Supabase Database Webhook deliveries to the `notify-on-write` Edge Function (Phase 24.1, ADR-017). Generated by the maintainer (`openssl rand -hex 32`) and registered both as a Supabase Edge Function secret AND as the signing secret on each Database Webhook in the Dashboard. The Edge Function verifies every delivery via constant-time HMAC-SHA256 compare.

**CONSUMED BY**: [`supabase/functions/notify-on-write/index.ts`](../../supabase/functions/notify-on-write/index.ts) — verifies the `x-supabase-webhook-signature` header on every webhook delivery. Three Database Webhooks fire this function (see [`supabase/webhooks.md`](../../supabase/webhooks.md)).

**OBTAIN**: generated locally with `openssl rand -hex 32` (any cryptographically random 32-byte hex string works; Supabase does not impose a length minimum but 256 bits aligns with HMAC-SHA256's recommended key size).

**REGISTER (Supabase Edge Function side)**:

```bash
SUPABASE_DATABASE_WEBHOOK_SECRET="$(openssl rand -hex 32)"
echo "$SUPABASE_DATABASE_WEBHOOK_SECRET"   # save this value — needed for Dashboard config below
...
**REGISTER (Database Webhook side)**: paste the same value into each Database Webhook's signing secret field via the Dashboard. Full step-by-step walkthrough: [`supabase/webhooks.md`](../../supabase/webhooks.md).

**ROTATE**: generate a new value with `openssl rand -hex 32`, re-register the Supabase secret, then update the signing secret on each of the 3 Database Webhooks via Dashboard. The webhook system does not auto-retry on signature mismatch (the Edge Function returns 401 and Supabase records the failure but does NOT re-deliver), so plan rotation during a quiet window or briefly tolerate missed pushes.
...
Expected (6 entries after Phase 24.1 `SUPABASE_DATABASE_WEBHOOK_SECRET` registration; was 5 after 2026-05-09 `verify-receipt` deletion):
...
SUPABASE_DATABASE_WEBHOOK_SECRET            # for notify-on-write Database Webhook signing (Phase 24.1)
```
...
| Edge Function `SUPABASE_DATABASE_WEBHOOK_SECRET` | `openssl rand -hex 32` for new value → re-register Supabase secret + update signing secret on each of the 3 Database Webhooks via Dashboard | Annual or on incident |
