...
| **A28 + A29** Observability alerts SOP | docs commit | New `docs/en/ops/observability-alerts.md` (+ JA mirror) — 5 Sentry alert configurations (crash-free sessions/users × 2 platforms + ANR) + 3 Play Vitals alerts (ANR + crash + slow cold start). Thresholds: 99% / 95% (alpha) → 99.5% / 99% (GA); ANR ≤ 0.47% / crash ≤ 1.09% (Play bad-behavior thresholds). Per-alert step-by-step Dashboard configuration, verification checklist, initial-response and tuning-loop guidance. User-side actions to configure during alpha launch readiness. |
