...
> **2026-05-09 amendment — `verify-receipt` deletion**: References to the `verify-receipt` Edge Function in this ADR are historical. The function was deleted on 2026-05-09 (agent-team deliberation: no complementary role given RevenueCat's coverage of Apple/Google receipt validation; client-initiated synchronous verification has no call site, defense-in-depth value low vs. additional Apple `.p8` + Google SA key custody surface, post-beta judgement preferred). The actual writer to `subscriptions` is now `revenuecat-webhook` (Phase 39 prep, 2026-05-08; calls `upsert_subscription_from_webhook` SECURITY DEFINER RPC from migration 023 with `last_verified_at` ordering guard). The architectural contract — service-role write, single writer, refund/cancellation via webhook — is unchanged; only the function identity changed. Apple App Store Server Notifications V2 + Google Play Real-Time Developer Notifications fan into RevenueCat (rather than Skeinly's own Edge Function), and RevenueCat fans into `revenuecat-webhook`. CANCELED/REFUNDED contract preserved per `supabase/functions/revenuecat-webhook/mapping.ts`. If post-beta surfaces a real need for an independent server-side validator (RevenueCat outage fallback, audit trail, etc.) it will land as a new "Phase H'" via a separate ADR.
