...
| `public.friend_connections` | **Wipe (asymmetric, ADR-024 §(g.1))** | `requester_id` / `user_a` / `user_b` | Added Phase 25.1; wipe behaviour deferred to migration 037 (Phase 25.1 follow-up). Per ADR-024 §(g.1) "wipe outbound, preserve inbound": **outbound** (`requester_id = auth.uid()`, any state) → `DELETE`; **inbound accepted** (participant, not requester, `state='accepted'`) → `UPDATE state='blocked', accepted_at=NULL` (so the original requester sees a clean severance signal); **inbound pending** (participant, not requester, `state='pending'`) → `DELETE`; **inbound blocked** → untouched (already terminal). Intentional asymmetry: connections the wiping user themselves initiated vanish with no signal; connections others initiated leave a "severed" marker. |
| `public.friend_invites` | **Wipe (outbound only)** | `inviter_id` | Added Phase 25.1; migration 037. `DELETE FROM friend_invites WHERE inviter_id = auth.uid()` — the user's own created invites are outbound artifacts (mirrors `user_blocks` / `shares` outbound posture). Rows where `consumed_by = auth.uid()` (an invite the wiping user redeemed, created by someone else) are **NOT touched**: the `friend_invites_consumed_pair` CHECK couples `consumed_at`/`consumed_by` so nulling `consumed_by` alone is invalid, and nulling both would resurrect a single-use invite as redeemable (security regression). The invite is the inviter's private artifact (RLS scopes reads to the inviter), so this matches the `user_blocks` inbound-preserve rationale. |
...
| 2026-05-15 | Preservation matrix amended with `friend_connections` + `friend_invites` rows (Phase 25.1 friend graph). Wipe semantics per ADR-024 §(g.1), applied via migration 037 (`wipe_own_data()` `CREATE OR REPLACE`). Closes the Tech Debt carried inline since migration 035. | architect (agent team) |
...
- ADR-024 §(g.1) — friend-graph wipe semantics (`friend_connections` / `friend_invites`); applied via migration 037.
