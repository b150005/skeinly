...
> **2026-05-09 amendment (1st)**: the original ADR cut described option (B) as "Supabase auto-signs the request with `webhook-secret` header (HMAC-SHA256)". This was inaccurate — per the [Supabase Database Webhooks doc](https://supabase.com/docs/guides/database/webhooks) and verified against the Dashboard UI, Database Webhooks do NOT auto-sign payloads. The Dashboard exposes only Method / URL or Edge Function / Timeout / HTTP Headers / HTTP Parameters; there is no signing-secret field and no signature header. Authentication is implemented via the custom-header path described above, which is the supported boundary. The decision (option B over A or C) stands; only the auth-mechanism details changed. See §3.9 for the full amendment.

> **2026-05-09 amendment (2nd)**: when configuring the 3 webhooks via Dashboard, select **`Supabase Edge Functions`** as the webhook type (NOT `HTTP Request`). Both types map to the same underlying `supabase_functions.http_request()` Postgres trigger function (verified by reading the function definition on prod via `pg_get_functiondef`); the trigger passes user-supplied HTTP Headers through to `net.http_post` unchanged, with no auth-injection logic of its own. The Edge Functions type is preferable on UX + maintenance grounds: (a) function selection by dropdown eliminates URL typo risk and survives function rename, (b) the webhook detail row in Dashboard shows "→ Edge Function: notify-on-write" rather than an opaque URL string. The Dashboard pre-populates the `Authorization` header with `Bearer <project anon key>` when this type is selected; **the maintainer MUST overwrite this with `Bearer <SKEINLY_DATABASE_WEBHOOK_SECRET>`** because the anon key is public (embedded in the Skeinly mobile app), which would let any app user POST a hand-crafted payload directly to the Edge Function URL and trigger fake push notifications (Phase 24.3+ once APNs/FCM credentials are wired). Envoy gateway's `/functions/v1/` route bypasses API-key validation (per [self-hosted Envoy gateway docs](https://supabase.com/docs/guides/self-hosting/self-hosted-functions)) so we cannot rely on gateway-level auth either.
...
> **2026-05-09 amendment**: the original cut of this ADR specified HMAC-SHA256 body-signature verification against an `x-supabase-webhook-signature` header. Verification of the [Supabase Database Webhooks doc](https://supabase.com/docs/guides/database/webhooks) and the actual Dashboard UI revealed that **Supabase Database Webhooks do NOT auto-sign payloads** — the only configuration surface exposed is Method / URL or Edge Function / Timeout / HTTP Headers / HTTP Parameters, with no signing-secret field and no signature header. Authentication is therefore implemented as a custom `Authorization: Bearer <SKEINLY_DATABASE_WEBHOOK_SECRET>` HTTP header, configured per-webhook in the Dashboard's HTTP Headers section, mirroring the `revenuecat-webhook` Bearer pattern (Phase 39 prep). The secret name `SKEINLY_DATABASE_WEBHOOK_SECRET` is unchanged. The Edge Function does constant-time string compare; no HMAC computation needed.

> **Webhook type selection**: the Dashboard offers two types — `HTTP Request` (manual URL) and `Supabase Edge Functions` (function dropdown). Choose `Supabase Edge Functions` for stronger maintenance posture (no URL to typo, dropdown survives function rename, Dashboard shows the function name not a URL). Both types map to the same `supabase_functions.http_request()` trigger and pass HTTP Headers through unchanged, so the Edge Function code path is identical. The Edge Functions type pre-populates `Authorization` with the project anon key — that value MUST be overwritten with `Bearer <SKEINLY_DATABASE_WEBHOOK_SECRET>` because the anon key is public (embedded in the mobile app); leaving it would let any app user POST hand-crafted payloads directly to the Edge Function URL and trigger fake push notifications. The Envoy gateway's `/functions/v1/` route bypasses API-key validation (per [self-hosted Envoy gateway docs](https://supabase.com/docs/guides/self-hosting/self-hosted-functions)), so we cannot rely on gateway-level auth either.
