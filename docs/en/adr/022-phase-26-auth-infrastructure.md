...
| 2026-05-14 | Amendment | claude-opus-4-7[1m] | Phase 26.6 ship-time clarification: §6.5 in the original cut said "MainActivity is a `ComponentActivity` which extends `FragmentActivity`". This is **INVERTED** — `FragmentActivity` extends `ComponentActivity` (verified in androidx.activity sources). `BiometricPrompt(FragmentActivity, Executor, AuthenticationCallback)` requires a `FragmentActivity`, so Phase 26.6 pivots `MainActivity : ComponentActivity()` → `MainActivity : FragmentActivity()`. All existing surfaces (`setContent`, `enableEdgeToEdge`, `registerForActivityResult`, `rememberNavController`, etc.) keep working because `FragmentActivity` extends `ComponentActivity`. Also: the original §6.5 cut described "primary biometric login" as a future option requiring 200–300 LOC. Phase 26.6 ships only the re-auth + sensitive-action surfaces; primary biometric login stays deferred to a separate ADR per the original scope. |
