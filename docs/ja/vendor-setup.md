...
4. **Service Account Credentials**: [release-secrets EF-4](release-secrets.md#ef-4-google_play_iap_validator_sa_json) で `GOOGLE_PLAY_IAP_VALIDATOR_SA_JSON` に使った `revenuecat@...` SA の JSON ファイルをアップロード (single source of truth — RevenueCat は product + receipt validation のために Play Developer API read access が必要)。
