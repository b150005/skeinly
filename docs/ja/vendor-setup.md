...
**ファイルとドキュメント** (1/1 チェック):
- ☑ **ファイルとドキュメント** — チャートデータ / パターンデータエクスポート (UGC)。
...
予想チェック合計: 14 カテゴリ全体で 10 件。Bug report 内容は Play の category 体系に clean な fit がない (Play の「メッセージ」サブタイプは SMS / email-style で、bug report は「アプリのアクティビティ > その他の操作」が近いが GitHub Issue + GitHub App 経由という destination が独立 lane)。提出時に operator 判断 — 最も誠実な fit を選ぶ、あるいは Play が両 route とも reject する場合は未申告で進める。
