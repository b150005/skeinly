...
- [ ] **保存** → Apple がチェックボックスごとに 6 択の用途 follow-up モーダルを表示 (「該当するものをすべて選択」)

**用途の選択肢** (6 択、全データタイプで共通):

| Apple オプション | 定義 (Apple 文言) | Skeinly での使用? |
|---|---|---|
| サードパーティ広告 | サードパーティ広告の表示 / サードパーティ広告事業体とのデータ共有 | **なし** (広告なし) |
| デベロッパの広告またはマーケティング | 自社広告、直接マーケティング、自社広告事業体とのデータ共有 | **なし** (transactional auth メールは marketing 非該当) |
| アナリティクス | 既存機能の有効性理解、新機能計画、視聴者規模・特性測定など、ユーザ行動の評価 | **限定的** — 下表参照 |
| 製品のパーソナライズ | おすすめ製品 / 投稿 / 提案などユーザ向け内容のカスタマイズ | **なし** (Discovery は最近性・activity-rank 表示、per-user 推薦なし) |
| アプリの機能 | ユーザ認証、機能の有効化、不正行為の防止、セキュリティ、サーバ稼働時間、**クラッシュの最小化**、**拡張性とパフォーマンス**、**カスタマーサポートの実行** | **あり** — 下表参照 |
| その他の目的 | 記載されていないその他の目的 | **なし** |

**チェックボックスごとの選択** (各項目は 6 択の意図的なサブセット; Apple は過剰申告も過少申告もペナルティ対象):

| データタイプ | 選択する用途 | 理由 |
|---|---|---|
| 名前 (display_name) | アプリの機能 | アカウント管理 + UGC 帰属表示 |
| メールアドレス | アプリの機能 | Supabase Auth login のみ; transactional auth メールは Apple 定義の "marketing" 非該当 |
| 写真またはビデオ | アプリの機能 | プロジェクト進捗写真機能 |
| カスタマーサポート | アプリの機能 | Apple の定義自体が「カスタマーサポートの実行」を明示 |
| その他のユーザコンテンツ | アプリの機能 | UGC (chart / pattern / comment / suggestion) はコア機能 |
| ユーザID (Supabase UUID) | アプリの機能 | 純粋な auth identifier。`PostHog.identify(supabaseUid)` は意図的に **未配線** (CLAUDE.md「strict anonymity stance」) — User ID は analytics ストリームに流入しない |
| デバイスID | **アプリの機能 + アナリティクス** | APNs token = push routing (アプリの機能) + PostHog `distinct_id` = device-level analytics linkage (アナリティクス) の 2 用途 |
| 購入 | アプリの機能 | RevenueCat subscription state で Pro 機能 gating |
| 製品の操作 | **アナリティクス** | PostHog の page view / tap / scroll は純然たる product analytics |
| クラッシュデータ | アプリの機能 | Apple の定義自体が「アプリのクラッシュの最小化」を明示 — アナリティクスではない (アナリティクスはユーザ行動評価、クラッシュデータはアプリ挙動の diagnostics) |
| パフォーマンスデータ | アプリの機能 | Apple の定義自体が「拡張性とパフォーマンスの向上」を明示 — クラッシュデータと同じ理由 |

- [ ] 各データタイプに対する 2 つの横断質問:
  - **Linked to user?** 全項目 **Yes** (User ID がグラフ全体の anchor; PostHog `distinct_id` は device-level だがセッション相関で Skeinly アカウントに紐付け可能)
  - **Used for tracking?** 全項目 **No** (クロスアプリ広告目的の第三者共有なし; Sentry / PostHog / RevenueCat / GitHub への SDK 経由転送は Apple 定義の service-provider 処理に該当)
