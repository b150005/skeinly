...
**メッセージ** (0/3): チェックなし。Skeinly にチャット / SMS 様メッセージング機能なし — パターンやコメントは forum-style UGC で別カテゴリで申告。
...
**連絡先** (0/1): チェックなし。Skeinly は連絡先リストにアクセスしない。

**アプリのアクティビティ** (1/6 チェック):
- ☑ **アプリでの操作** — PostHog の page view / tap / scroll (同意画面 opt-in)。
- チェック外す: アプリ内検索履歴 (transient で永続化しない); インストール済みのアプリ; その他のユーザー生成コンテンツ (user content 側で申告); その他の操作 (取得は PostHog の app actions に閉じる)。

**ウェブ閲覧履歴** (0/1): チェックなし。ブラウジング履歴は取得しない。

**アプリの情報とパフォーマンス** (2/3 チェック):
- ☑ **クラッシュログ** — Sentry crash logs (opt-in)。
- ☑ **診断** — Sentry performance traces (opt-in)。
- チェック外す: その他のアプリパフォーマンスデータ。

**デバイスまたはその他の ID** (1/1 チェック):
- ☑ **デバイスまたはその他の ID** — FCM/APNs push token + PostHog distinct_id。

予想チェック合計: 14 カテゴリ全体で 10 件。

**Bug report 内容には Play の taxonomy に clean な fit がない**:
- メッセージ > その他のアプリ内メッセージ — Play の意図は user-to-user リアルタイム / DM messaging で、user-to-developer support submission ではない
- メッセージ > メール — bug body は「ユーザーのメール内容」ではない
- アプリのアクティビティ — PostHog で取得しているイベントに bug body は含まれない
- 個人情報 > その他の情報 — 定義の拡大解釈

**推奨: 未申告** で Play Data Safety を進める。Bug report フローは privacy policy で既に開示済 (Phase 39 W5b — Edge Function proxy + Skeinly Feedback GitHub App + Issue body data)。Phase 40 GA 提出時に Play フォーム審査で flag されたら再検討。
