...
**ファイル、ドキュメント** (0/1): チェックなし。Skeinly のチャートデータは構造化 UGC であり、伝統的な「ファイル」(PDF / spreadsheet / document) ではない。このカテゴリは Skeinly のチャートに対して loosely fit するのみ、より精密な fit は アプリのアクティビティ > その他のユーザー作成コンテンツ (下記参照)。double-counting 回避のため未チェック。
...
**アプリのアクティビティ** (3/5 チェック):
- ☑ **アプリのインタラクション数** — PostHog の generic な page view / tap / scroll (同意画面 opt-in)。
- ☑ **その他のユーザー作成コンテンツ** — 編み図 (`chart_documents` の structured records)。Play の tooltip 「この一覧または他のセクションに記載されていない、その他のユーザー作成コンテンツ」に該当 — Skeinly のチャートデータは structured UGC で、写真 / 動画 / ファイル / 音声 のいずれにも fit しない。
- ☑ **その他の操作** — PostHog の Skeinly 固有カスタムイベント (`pattern_created` / `pattern_shared` / `suggestion_opened` 等)、generic interaction count とは別軸の app-specific user actions。Play の tooltip 「ここに記載されていないその他のユーザー アクション」に該当。
- チェック外す: アプリ内の検索履歴 (transient で永続化しない); インストール済みのアプリ (他アプリ調査機能なし)。
...
予想チェック合計: **14 カテゴリ全体で 12 件** (Phase 28 完了後は 動画 追加で 13 件)。

#### ページ 4: データの使用と処理

ページ 3 でチェックした 11 種それぞれに対し、Play が per-type フォームで以下を聞く:
1. **収集 / 共有** — 収集のみ vs サードパーティ共有 (両方可)
2. **一時的処理か** — はい (RAM-only、永続化なし) / いいえ (永続化)
3. **必須 vs オプション** — signup 必須 or ユーザー opt-out 可能
4. **収集の用途** — 最大 7 checkbox
5. **共有の用途** — Q1 で 共有 がチェックされた場合のみ

Skeinly 共通パターン: **収集 のみ** (全 11 種で 共有 = なし。Sentry / PostHog / RevenueCat / GitHub / APNs / FCM は Play の「service provider 例外」に該当しサードパーティ共有ではない)、**一時的処理 = いいえ** (全て Supabase または service provider 側で永続化)。残り 2 軸の per-type マトリクス:

| データ種類 | 必須/オプション | 用途 (チェックする項目のみ) |
|---|---|---|
...
| クラッシュログ | ユーザーは...選択できる (opt-in 同意) | **分析** (Sentry 自動 crash 収集; bug report 機能の本文は その他のアプリ内メッセージ で扱う別 data type) |
| 診断情報 | ユーザーは...選択できる (opt-in 同意) | **分析** |
| デバイスまたはその他の ID | ユーザーは...選択できる (push 許可 + PostHog opt-in) | アプリの機能 + **分析** |

**名前 / メール / ユーザー ID / 購入履歴 で デベロッパー コミュニケーション をチェックする理由**:
- 名前 / メール / ユーザー ID — Supabase Auth が送る transactional メール (email verification / password reset / 将来のサポート返信メール等) でパーソナライズ (受信者ごとに display_name を本文記載) を実施。受信者名の本文記載は迷惑メール判定回避にも寄与する legitimate な developer-to-user 通信。ユーザー ID 経由で特定ユーザーへのメール送信先解決。
- 購入履歴 — Pro 加入者向けに「Pro 機能新機能告知」push 通知を送る。Play の category description「アプリの新機能をユーザーに通知する」に明確該当。

**全 12 種でチェックしない用途**:
- ☐ **広告、マーケティング** — Skeinly は広告なし
- ☐ **不正行為防止、セキュリティ、コンプライアンス** — fraud-detection 専用処理なし
- ☐ **カスタマイズ** — personalized recommendation 機能なし (Discovery は activity-rank、per-user カスタマイズなし)

**よくある混乱**: クラッシュログ data type は **Sentry の自動 crash 取得** が対象であり、**ユーザーが Settings → Send Feedback で能動的に書く bug report 本文ではない**。Bug report 本文は **メッセージ > その他のアプリ内メッセージ** に分類済。クラッシュログ / 診断情報 (どちらも Sentry 自動) の用途は Play の **分析** description「バグやクラッシュを診断して修正する、将来的にパフォーマンスの改善を加える」に正確 fit するため **分析 のみ**。アプリの機能 のチェックは その他のアプリ内メッセージ form (bug report 本文の data type) 側でカバー。

ページ末尾の security practices セクション (11 種すべて完了後に表示): 送信時暗号化 **はい**、ユーザー削除要求可能 **はい** (in-app + web — Phase 27 ship 後に `data-deletion/` URL も)、独立検証 **いいえ**、Families Policy **いいえ**。
