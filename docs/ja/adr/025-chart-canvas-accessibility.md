...
- 2026-05-18 — **R1c 出荷**（編み図比較＋行単位変更リスト）。`ChartAccessibility` を拡張: `DiffChangeKind`（`ADDED` / `REMOVED` / `MODIFIED`）、`RowDiffChange`（1-based `colNumber`、kind、symbolId）、`RowDiffDescriptor`（1-based `rowNumber` / `rowCount` / `chartY` / 非空 `changes` リスト）、`DiffA11yStrings`（7 件のローカライズ済みテンプレート — 3 件は R1a の `a11y_chart_*` を再利用、4 件は R1c 新規）、純粋 `rowDiffDescriptors(targetExtents, cellChanges, layerChanges)`（`CellChange.Added`/`Removed`/`Modified` ＋ `LayerChange.Added`/`Removed` のセルを chartY ごとの変更リストにまとめる; `LayerChange.PropertyChanged` はレンダラ既存ルールに合わせてセル単位では無視; 変更は `colNumber` 昇順にソート; 変更が 1 件以上ある行のみ出力 — SR ユーザーが無変更行を延々とスワイプしないように）、`spokenDiffLabel(...)` 両プラットフォームでバイト同一の整形 ＋ 新規 12 件 `commonTest`（kind ごとの chartY マッピング、layer-add/layer-remove 展開、PropertyChanged 無視、列昇順ソート、空行省略、オフセット extents での 1 始まり行/列、target 範囲外変更の silent drop — 縮小ターゲット rare ケース、退化 extents → 空、Modified は AFTER シンボル、整形の null/未解決 id フォールバック）— `ChartAccessibilityTest` 合計 **47 件 GREEN**。Compose `RectComparisonAccessibilityOverlay`（`ChartComparisonScreen.kt` 内 private composable — `BoxWithConstraints` で target ペインの px サイズを露出し、視覚 Canvas が描くのと同じ前方 `computeDiffLayout` 計算でベーススケールに行毎 invisible `Box` を配置; 親に `isTraversalGroup` ＋ 各行に `traversalIndex = rowNumber` で行 1 先頭の SR トラバーサル）＋ SwiftUI `ChartComparisonAccessibilityOverlay`（新規 `iosApp/iosApp/Screens/ChartComparisonAccessibility.swift`、R1b の `ChartEditorAccessibility.swift` をミラー; 行毎 `Color.clear` ＋ `.accessibilityElement()` ＋ `.accessibilityLabel` ＋ `.accessibilitySortPriority`）を `ChartComparisonScreen.swift` `DualCanvasPanel` に配線 — target ペインを `GeometryReader` ＋ `ZStack` でラップ; `.accessibilityHidden(true)` は内側 `DiffCanvas` のみに適用し、`targetChartCanvas` accessibilityIdentifier を持つ `GeometryReader` ラッパーには適用しない（§(d) 実装時の精緻化 — Maestro ランドマーク保持）。実装時の精緻化（決定は不変）: (i) オーバーレイは TARGET ペインのみに整合（`diff.base` は初回コミットで `nil` になりうる; `target` は常に非 null かつ「現在この位置にあるもの」を表す — `spokenDiffLabel` は post-change state を基点に統合差分を読み上げる）; (ii) `LayerChange.PropertyChanged` は既存 `LayerChangesBanner` のみで公開し、オーバーレイのセル単位列挙からは意図的に除外（レンダラの `classifyCells` ルールと整合）; (iii) Removed セルで `chartY` が `target.extents` 範囲外（rare な縮小ターゲットケース）のものは読み上げから silent drop — base ペインには視覚的に残り、統合行インデックスで追跡すると SR ユーザーに範囲外の幻の行番号を押し付けることになる（監査が再訪する場合の Follow-up として記録）; (iv) R1c の新 i18n キー 4 件（`a11y_diff_change_added` / `_removed` / `_modified` / `_separator`）は `docs/en/phase/tasks/R1c.i18n.tsv` 経由でオーケストレータがコンソリデーション時に共有ファイルへスプライス（並行ワークツリー i18n-fragment プロトコル）; Compose ＋ SwiftUI 両方がデバイスロケールで切替えるソース内 en/ja バイリンガルフォールバックを使用 — R1b の `makeCellA11yStrings(isJa:)` と同一パターン。監査 B4 → CLOSED (R1c)。カラー以外で区別（Differentiate-Without-Color）は本サブスライスで申告可能化; VoiceOver は監査 §5 進行表により引き続き R2（アイコンラベル）＋ R3（見出し）も必要 — R1 は VoiceOver に必要だが十分でない。
