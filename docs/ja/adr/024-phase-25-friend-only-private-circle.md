...
プライバシーポリシー `<h3>` サブセクションを `friend_connections` データ + visibility セマンティクスに追加。EN + JA ミラー。内容：収集されるデータ（`(user_a, user_b, state, requester_id, timestamps)` のみ — プロフィールデータなし）、保持セマンティクス（決定 (g.1) 通り、データ消去を越えて保持）、GDPR 権利列挙拡張（「Settings → Privacy → Connections からフレンド接続を管理」）。
