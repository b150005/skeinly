...
> **非推奨**: Path Y(Fastfile commit `0956d3f`、2026-05-11)以降、
> CI はこの secret を読み取りません。プロファイルは実行時に `sigh`
> アクション(App Store Connect API key 5–7 を使用)が Apple Developer
> Portal から fetch します。capability 変更時は Portal Web UI で
> プロファイルを 1 回再生成するだけで、次の CI run が自動取得します。
> GitHub Secret の更新は不要になります。
>
> **この secret は削除して問題ありません**(未使用 credential の attack
> surface 削減のため):
>
> ```bash
...
> ```
>
> 注意: プロファイル本体は Apple Developer Portal 上に存続する必要が
> あります(sigh が Portal から fetch するため)。obsolete なのは
> GitHub Secret に格納されていた base64 化バイト列のみです。以前の
> OBTAIN/VERIFY/REGISTER 手順は履歴参考のため下に折りたたんで残します。

<details>
<summary>履歴手順(Path Y 以前、参考用)</summary>
...
> **命名**: `SKEINLY_` プレフィックスは load-bearing (EF-6 / EF-3 と同じ理由)。
...
Path Y 非推奨化 2026-05-11 以前は `APPLE_PROVISIONING_PROFILE_BASE64`
を含めて 19 エントリでした):
...
> `APPLE_PROVISIONING_PROFILE_BASE64` がまだ `gh secret list` に残っている
> 場合、削除しても問題ありません — Path Y で CI は Apple Developer Portal
> から実行時に fetch するよう移行済。§3 参照。
...
期待される出力（Phase 39 W5a で `SKEINLY_BUGREPORT_*` 3 件登録後は 9 エントリ。Phase 24.1 `SKEINLY_DATABASE_WEBHOOK_SECRET` 登録後は 6、2026-05-09 `verify-receipt` 削除以降は 5、それ以前は 9）:
...
| `APPLE_PROVISIONING_PROFILE_BASE64` (DEPRECATED 2026-05-11) | GitHub Secret ローテーション不要 — sigh が毎 CI run で Apple Developer Portal から fetch。Portal Web UI での手動 re-Generate は capability 追加時 + 年次失効更新時のみ。 | GitHub 側 N/A; Portal の re-Generate は capability 追加 + 年次失効時のみ |
