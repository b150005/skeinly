...
5. プロンプトで**パスワードを設定**（任意の文字列、例えば `openssl rand -hex 16` の出力）。このパスワードを保存してください — `IOS_DISTRIBUTION_CERT_PASSWORD` として使います。
6. base64 エンコード:
...
| シークレット | ローテーション手順 | 頻度 |
|---|---|---|
| `IOS_DISTRIBUTION_CERT_BASE64` + パスワード | Apple Developer → Certificates → revoke + 新規作成 + `.p12` 再エクスポート | 年次またはインシデント時 |
| `IOS_PROVISIONING_PROFILE_BASE64` | Apple Developer → Profiles → 再生成（同じ cert） | Apple により年次強制 |
