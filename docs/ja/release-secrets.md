...
期待される出力（Phase 24.1 で `SKEINLY_DATABASE_WEBHOOK_SECRET` 登録後は 6 エントリ。2026-05-09 `verify-receipt` 削除以降は 5、それ以前は 9）:
...
| Edge Function `SKEINLY_DATABASE_WEBHOOK_SECRET` | `openssl rand -hex 32` で新規生成 → Supabase secret 再登録 + Dashboard 上の 3 つの Database Webhook の signing secret を全部更新 | 年次またはインシデント時 |
