...
このドキュメントは、リリースパイプラインおよび Supabase Edge Function が消費する全シークレットの取得・検証・登録手順を段階的にまとめたガイドです。**GitHub Secrets 21 種**（うち Firebase 2 種は `production` / `development` Environment 別に 2 回登録するため**合計 23 登録**）を 7 カテゴリ + **Supabase Edge Function ランタイムシークレット 5 個**を 8 番目のカテゴリで扱います:
...
- **Supabase ランタイム**（2 個、Repo）— バックエンド URL + anon key（値は Publishable key 形式 `sb_publishable_...` 推奨。Secret 名 `SUPABASE_ANON_KEY` はコード側参照との整合のため当面維持）
...
- [Supabase Edge Function Secrets（5 シークレット）](#supabase-edge-function-secrets4-シークレット)
- [一括検証](#一括検証)
...
### 13. `SUPABASE_ANON_KEY`（値は新形式 Publishable key を使う）

> **2025-11-01 値フォーマット移行**: legacy `anon` JWT (`eyJ...`) は廃止予定。**Secret 名は当面 `SUPABASE_ANON_KEY` のまま**（CI workflow + Gradle build + iOS Info.plist injection が現名を参照しているため）だが、**値だけを新形式 Publishable key (`sb_publishable_...`) に差し替える**。
>
...
>
> **将来の Secret 名リネーム** (`SUPABASE_ANON_KEY` → `SUPABASE_PUBLISHABLE_KEY`) は別 PR で `shared/build.gradle.kts` / `shared/src/iosMain/.../SupabaseConfig.ios.kt` / `.github/workflows/{release,ci,e2e}.yml` の参照を一括更新するときに行う。

**内容**: 「Publishable」公開 API キー。クライアントアプリへの埋め込みを想定（RLS ポリシーがデータベース層で不正アクセスを防ぐ）。形式は不透明トークン (`sb_publishable_<base62>`)。**JWT ではない** — レガシー anon key とは値の形式自体が異なる。Supabase SDK は両形式とも受理するため、Secret 名 `SUPABASE_ANON_KEY` のままでも値だけ差し替えれば動作する。
...
> **note**: `SUPABASE_ANON_KEY` は legacy 名のまま値だけ Publishable key (`sb_publishable_...`) に差し替える。Secret 名のリネームは別 PR で workflow + Gradle + iOS code の参照を一括更新する際に実施。

期待される **`production` Environment scope** 出力（2 エントリ — Firebase prod プロジェクト由来）:
...
合計: Repository 19 + Environment scope (production: 2 + development: 2) = **23 登録**。欠けているものや古いタイムスタンプのものは要確認。
...
>
> `SUPABASE_ANON_KEY` は **削除しない** — Secret 名のままで値だけ Publishable key に差し替える（コード側参照との整合のため）。
...
| `SUPABASE_ANON_KEY`（値は Publishable key 形式） | Supabase Dashboard → Project Settings → API Keys → Publishable key を Generate new key（Secret 名のリネームは別 PR） | 漏洩疑い時 |
