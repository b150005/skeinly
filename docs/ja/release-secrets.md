...
- [Supabase Edge Function Secrets（5 シークレット）](#supabase-edge-function-secrets5-シークレット)
- [一括検証](#一括検証)
...
**Webhook Secret について**: RevenueCat の Webhook 署名検証用 secret は **Edge Function 専用**（クライアントには不要）。本ドキュメント末尾の Edge Function Secrets セクション [EF-5 `REVENUECAT_WEBHOOK_SECRET`](#ef-5-revenuecat_webhook_secret) を参照。
...
## Supabase Edge Function Secrets（5 シークレット）

これらのシークレットは Supabase Edge Function (`notify-on-write` でコラボ push — Phase 24.1 (本 slice) が shell を配線、Phase 24.3 が実 APNs / FCM 送信、`revenuecat-webhook` で IAP webhook 受信 — Phase 39 prep, 2026-05-08) が消費します。**GitHub Secrets ではありません** — Supabase CLI で Supabase プロジェクトに登録します:
...
> **2026-05-09 DEPRECATED**: 削除済みの `verify-receipt` Edge Function 用に登録した secret。Agent team 協議の結果、RevenueCat が Apple/Google レシート検証を server-side で完結させているため (verify-receipt の補完的役割が確立できない)、`verify-receipt` 自体を削除しました。基底 SA `revenuecat@<project-ref>.iam.gserviceaccount.com` は RevenueCat dashboard 側で引き続き使用 (そちらの登録は影響なし)。Supabase secret 側はもうどの Edge Function からも消費されないため、credential surface 削減のため `supabase secrets unset GOOGLE_PLAY_IAP_VALIDATOR_SA_JSON` で削除可。本セクションは rotation 参照用 + post-beta で Phase H'相当 (独立 server-side validator) を再検討する場合の手順保存目的で残置。

**WHAT**: Google Play Developer API 用 Service Account JSON。Android IAP レシートのサーバー側検証および subscription 状態変更（更新、解約、返金）の観測用。元々は `supabase/functions/verify-receipt/index.ts` の `verifyGoogleReceipt` (2026-05-09 削除) から消費されていました。

**SA**: `revenuecat@<project-ref>.iam.gserviceaccount.com`（RevenueCat dashboard と同じ SA を再利用 — single source of truth）。

> **2026-05-08 マイグレーション**: 旧 secret 名 `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` から `GOOGLE_PLAY_IAP_VALIDATOR_SA_JSON` に rename + SA を `google-play-publisher@...` から `revenuecat@...` に切り替え（PoLP 分離）。`google-play-publisher@...` は CI Release アップロード専用に再配置（[#21 `GOOGLE_PLAY_PUBLISHER_SA_JSON_BASE64`](#21-google_play_publisher_sa_json_base64) 参照）。マイグレーション手順は本セクション末尾。

**OBTAIN:**

すでに `revenuecat@...` SA が Cloud Console に存在し、JSON 鍵も発行済みのはずです（vendor-setup.md A0c-3 RevenueCat 連携時に作成）。再発行が必要な場合のみ:

1. [Google Cloud Console](https://console.cloud.google.com) → IAM と管理 → **サービス アカウント** → `revenuecat@...` をクリック
2. **キー** タブ → **キーを追加** → **新しいキーを作成** → JSON → ダウンロード
3. （任意）古い鍵を **無効化** + **削除**

**VERIFY**: JSON が `"type": "service_account"`、`client_email` が `revenuecat@<project-ref>.iam.gserviceaccount.com`。Play Console **ユーザーと権限** ページに同 email が登録済みで、以下の権限が ✅:
- 売上データ、注文、解約アンケートの回答の閲覧（**View financial data**）
- 注文と定期購入の管理（**Manage orders**）
- アプリ情報の閲覧、一括レポートのダウンロード（読み取り専用）（自動）

> **権限スコープ**: アプリ単位 / アカウント単位どちらでも可。Skeinly が単一アプリの間は機能差なし。将来複数アプリを管理する場合は、Skeinly 単位に絞る方が PoLP 厳密。
...
**マイグレーション手順**（旧 `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` 名で `google-play-publisher@...` JSON が登録済みの場合）:

```bash
# 1. 旧 secret 削除
supabase secrets unset GOOGLE_PLAY_SERVICE_ACCOUNT_JSON

# 2. 新 secret 名 + 新 SA で登録
supabase secrets set GOOGLE_PLAY_IAP_VALIDATOR_SA_JSON="$(cat ~/path/to/revenuecat-sa.json)"

# 3. 確認
supabase secrets list | grep GOOGLE_PLAY
# GOOGLE_PLAY_IAP_VALIDATOR_SA_JSON のみが表示されるはず
```

> Edge Function `verify-receipt` は 2026-05-09 に削除されました。上記マイグレーション手順は legacy 名で登録済みの場合のみ参考に。新旧どちらの名前でも `unset` で同等。

**ROTATE**: Cloud Console → IAM → Service Accounts → `revenuecat@...` クリック → Keys → 古い key を revoke + 新規 JSON 作成。**3 箇所の secret を再登録**:
1. Supabase Edge Function: `GOOGLE_PLAY_IAP_VALIDATOR_SA_JSON`
2. RevenueCat dashboard: Project Settings → Apps → Skeinly → Service Account Credentials を再アップロード
3. （該当すれば）他に同 JSON を消費する箇所があれば
...
### EF-6. `SUPABASE_DATABASE_WEBHOOK_SECRET`

**WHAT**: Supabase Database Webhook が `notify-on-write` Edge Function に配信するペイロードの署名検証用共有秘密値 (Phase 24.1, ADR-017)。Maintainer が `openssl rand -hex 32` で生成し、Supabase Edge Function secret として AND Dashboard 上の各 Database Webhook の signing secret として **両方** に登録。Edge Function は受信した webhook を `x-supabase-webhook-signature` ヘッダの値と HMAC-SHA256 定数時間比較で検証。

**消費箇所**: [`supabase/functions/notify-on-write/index.ts`](../../supabase/functions/notify-on-write/index.ts) — webhook 配信ごとに `x-supabase-webhook-signature` ヘッダ検証。3 つの Database Webhook がこの Edge Function を fire する (詳細は [`supabase/webhooks.md`](../../supabase/webhooks.md))。

**OBTAIN**: ローカルで `openssl rand -hex 32` 生成 (任意の暗号論的乱数 32 byte hex 文字列で可、Supabase 側に最小長制約はないが HMAC-SHA256 推奨鍵長 256-bit に揃える)。

**REGISTER (Supabase Edge Function 側)**:

```bash
SUPABASE_DATABASE_WEBHOOK_SECRET="$(openssl rand -hex 32)"
echo "$SUPABASE_DATABASE_WEBHOOK_SECRET"   # この値を控える — Dashboard 設定に使う
supabase secrets set SUPABASE_DATABASE_WEBHOOK_SECRET="$SUPABASE_DATABASE_WEBHOOK_SECRET"
supabase secrets list | grep SUPABASE_DATABASE_WEBHOOK_SECRET
```

**REGISTER (Database Webhook 側)**: 同じ値を Dashboard 上の各 Database Webhook の signing secret 欄にペースト。step-by-step は [`supabase/webhooks.md`](../../supabase/webhooks.md) を参照。

**ROTATE**: `openssl rand -hex 32` で新しい値を生成、Supabase secret を再登録、Dashboard 上の 3 つの Database Webhook 全部の signing secret を更新。Webhook システムは署名 mismatch で auto-retry しない (Edge Function が 401 を返し Supabase は失敗を記録するが re-deliver しない) ため、ローテーション中は静かなタイミングで実施するか、瞬間的な push 取りこぼしを許容する。
...
### 再利用: App Store Connect API キー (DEPRECATED 2026-05-09 for Edge Functions)

> **2026-05-09 Edge Function 消費は DEPRECATED**: 削除済みの `verify-receipt` Edge Function (iOS 分岐、App Store Server API 呼出時の JWS 署名用) のために登録した secret。RevenueCat が iOS レシート検証を server-side で完結させているため、Supabase 側の本 3 secret はもうどの Edge Function からも消費されません。安全に削除可:
>
> ```bash
...
> ```
>
> 同じ App Store Connect API キーは iOS リリースパイプライン (`xcrun altool` 経由 TestFlight アップロード) のために GitHub Secrets §5–§7 として **引き続き必須** です。

歴史的経緯: `verify-receipt` が IAP 検証経路だった時代は、本 Supabase Edge Function 登録が GitHub Secret と並ぶ single source of truth として必要でした。
...
期待される **Repository scope** 出力（17 エントリ — 全環境共通の値）:
...
合計: Repository 19 + Environment scope (production: 3 + development: 2) = **24 登録**。欠けているものや古いタイムスタンプのものは要確認。
...
>
> Supabase Edge Function 側 legacy 名のクリーンアップ:
> - `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON`（→ `GOOGLE_PLAY_IAP_VALIDATOR_SA_JSON` に rename + SA を `revenuecat@...` に切り替え、2026-05-08）。`supabase secrets unset GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` で削除。
...
期待される出力（Phase 24.1 で `SUPABASE_DATABASE_WEBHOOK_SECRET` 登録後は 6 エントリ。2026-05-09 `verify-receipt` 削除以降は 5、それ以前は 9）:
...
SUPABASE_DATABASE_WEBHOOK_SECRET            # notify-on-write Database Webhook 署名 (Phase 24.1)
```

旧 4 secret (`APP_STORE_CONNECT_API_KEY` + `APP_STORE_CONNECT_KEY_ID` + `APP_STORE_CONNECT_ISSUER_ID` + `GOOGLE_PLAY_IAP_VALIDATOR_SA_JSON`) が登録済みの場合、現在は dormant。EF-4 と「再利用: App Store Connect API キー」セクションの DEPRECATED 注記内に `unset` コマンドあり。
...
| `APP_STORE_CONNECT_API_KEY_*` | App Store Connect → Team Keys → revoke + 新規生成（Supabase Edge Function 側は 2026-05-09 `verify-receipt` 削除以降 dormant — Phase H' 等で server-side validator が復活した時のみ再登録） | 12 ヶ月ごと推奨 |
...
| Edge Function `GOOGLE_PLAY_IAP_VALIDATOR_SA_JSON` (DEPRECATED 2026-05-09) | `verify-receipt` 削除以降 Supabase secret は dormant。Cloud Console → IAM → Service Accounts → `revenuecat@...` → revoke + 新キー → RevenueCat dashboard (現役の唯一の消費先) を再アップロード | 年次またはインシデント時 |
...
| Edge Function `SUPABASE_DATABASE_WEBHOOK_SECRET` | `openssl rand -hex 32` で新規生成 → Supabase secret 再登録 + Dashboard 上の 3 つの Database Webhook の signing secret を全部更新 | 年次またはインシデント時 |
