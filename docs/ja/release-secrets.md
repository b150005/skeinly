...
**WHAT**: Google Play Developer API 用 Service Account JSON。Android IAP レシートのサーバー側検証および subscription 状態変更（更新、解約、返金）の観測に使います。**Phase H で `supabase/functions/verify-receipt/index.ts` の `verifyGoogleReceipt` から消費**されます（現状はスタブが 501 を返すだけで未実装、secret 自体は dormant）。
...
> Edge Function 側コード (`verify-receipt/index.ts:21`) も同名へ更新済み。`verifyGoogleReceipt` 本体は Phase H 着手時に実装。
...
期待される出力（9 エントリ — Edge Function 固有 5 個 + App Store Connect API から再利用 4 個）:
...
| Edge Function `GOOGLE_PLAY_IAP_VALIDATOR_SA_JSON` | Cloud Console → IAM → Service Accounts → `revenuecat@...` → revoke + 新キー → Supabase secret + RevenueCat dashboard を再登録 | 年次またはインシデント時 |
