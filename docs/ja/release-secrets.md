...
これらのシークレットは Supabase Edge Function (`notify-on-write` で Push 送信 — Phase 24 で実装予定、`revenuecat-webhook` で IAP webhook 受信 — Phase 39 prep, 2026-05-08) が消費します。**GitHub Secrets ではありません** — Supabase CLI で Supabase プロジェクトに登録します:
...
期待される出力（2026-05-09 の `verify-receipt` 削除 + secret unset 後は 5 エントリ。削除前は 9 エントリ）:

```
APPLE_APNS_KEY_ID                           # notify-on-write 用 (Phase 24, 未実装)
APPLE_APNS_KEY_P8                           # notify-on-write 用 (Phase 24, 未実装)
APPLE_TEAM_ID                               # notify-on-write 用 (Phase 24, 未実装)
FIREBASE_SERVICE_ACCOUNT_JSON               # notify-on-write 用 (Phase 24, 未実装)
