...
各テスターから以下を確認:
- メールアドレス（sandbox 購入時に App Store / Google Play サインインに使うもの — Skeinly ログインのメールと別でも OK、プラットフォーム側の sandbox アカウント）。Phase 39 では、オペレータが Gmail プラスサブアドレッシングを `skeinly.app+<cohort>-<locale>-<label>@gmail.com` パターンで中央集約 inbox に向けて運用するのが標準 — メールパターン詳細は [vendor-setup.md](../vendor-setup.md#a0b-9-sandbox-tester) A0b-9 参照
