...
base plan 行の 3 点リーダー ⋮ メニューに見える項目は secondary actions のみで、draft base plan の場合は通常 **サポートを終了した請求方法で使用** (`Use for deprecated billing methods` = backwards compat マーク) しか並ばない。**アクティブ化は ⋮ メニューには存在しない** — 行テキスト click → edit page → 最下部ボタン、というパス。⋮ メニューに「サポートを終了した請求方法で使用」しか見えなくても正常で、そちらは触らない。
...
**アクティブ化** クリック (月額と同じパス、ステップ 2d 参照: `yearly` の ID テキストをクリック → edit page → 最下部の「有効にする」/「アクティブ化」 ボタン。3 点リーダー ⋮ メニュー経由ではない)。
...
### Add phase / 「フェーズを追加」 の場所

offer ID + base plan 関連付け + eligibility を入力した後、**同じ offer 作成フォーム内**を下にスクロールすると **「フェーズ」/「Phases」** セクションがある。**「フェーズを追加」/ Add phase** ボタンはそこに位置 — **別ページではない**。offer を先に保存する必要なし。

**フェーズを追加** クリック → 3 つのフェーズタイプが出現:
- **無料トライアル (Free trial)** — 指定期間ゼロコスト (最小 3 日、最大 3 年)
- **単一払い (Single payment)** — 割引一括前払い
- **継続支払い (Recurring payment)** — N 期間にわたる割引継続課金

**無料トライアル** を選択 → Duration を **7 days** に設定。Free Trial フェーズには価格フィールドが表示されない — ゼロコストが定義上自明。

### オファーのアクティブ化

**保存** クリックで offer を保存。続けて offer edit page 最下部の **アクティブ化** をクリックで購入可能化。Offer の Activate ボタンは base plan の Activate ボタンとは独立 (base plan がアクティブでも offer は draft 状態にしておけるし、その逆も可)。

> **2026-05-13 検証済 (Google Play Console Help)**: base plan は **アクティブ化前でも offer 作成可能** — draft (未公開) 状態の base plan でも offer 作成フォームは普通に動作する。Country availability の事前設定不要 (並列配置)。Offer 自体はアクティブ化しないと購入できない。
