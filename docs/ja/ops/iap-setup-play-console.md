...
パス:
1. サブスクリプション詳細ページから `monthly` 行の **›** (右矢印) をクリック → base plan edit page を開く
2. ページ最下部までスクロール
3. **アクティブ化** (`Activate`) をクリック → ダイアログで確認。ステータスが **未公開** → **有効** に変化

base plan 行の 3 点リーダー ⋮ メニューに見える項目は secondary actions のみ (**従来の価格表示**、**サポートを終了した請求方法で使用** = backwards compat マーク等)。**アクティブ化は ⋮ メニューにはない** — 一段深い edit page にあるボタン。⋮ メニューに「サポートを終了した請求方法で使用」しか見えなくても正常。
